...
## Dataset
- **257 places across 157 real Kolkata localities.** Geography (area names + coordinates)
  is genuine; names, hours, verification dates and phone numbers are synthetic.
- Phone placeholders follow the sequence `+91 98300 0xxxx`.
- **CALL is intentionally disabled on every seed record.** `isPlaceholderPhone()` matches
  the placeholder sequence and `#btn-call` refuses. Do not re-enable — dialling a
  placeholder could reach a real stranger. Real numbers enable it automatically.
- Mix: mechanic 76, air 50, structural 37, control 36, parking 29, tow 29.
